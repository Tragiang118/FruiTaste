...
export default function AdminImportDetailPage() {
  ...
  const fetchImportDetails = async () => {
    try {
      ...
      // Fetch the actual import receipt details to get the creator name
      const [txRes, receiptRes] = await Promise.all([
        api.get('/inventory/transactions'),
        api.get(`/inventory/import/${importId}`).catch(err => {
          console.error("Failed to fetch import receipt details", err);
          return { data: null };
        })
      ]);
