    ...
    return (
      <div className="p-6 md:p-8 space-y-6">
        ...
        <div className="text-center py-20 text-gray-500 bg-white rounded-3xl border border-gray-100 shadow-sm">
          Đơn hàng không tồn tại hoặc có lỗi xảy ra.
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 md:p-8 space-y-6 w-full h-full overflow-y-auto bg-gray-50/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          {/* Timeline */}
          {order.status !== 'CANCELLED' && (
            <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden p-6">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden">
            <CardHeader className="p-6 border-b border-gray-50">
              <CardTitle className="text-base font-bold text-gray-700 ml-1 flex items-center gap-2">
                <Package size={16} className="text-primary" /> Sản phẩm đã đặt
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <table className="w-full text-sm">
                ...
                <tfoot className="bg-gray-50/30 border-t border-gray-100 font-bold">
                  <tr>
        ...
        <div className="space-y-6">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden p-6 space-y-4">
            <h4 className="text-sm font-bold text-gray-700 ml-1 flex items-center gap-2 border-b pb-3">
              <User size={14} className="text-primary" /> Thông tin giao hàng
            </h4>
            <div className="space-y-3">
              <div>
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Người nhận</span>
                <p className="text-lg font-black text-gray-900">{order.shippingName}</p>
              </div>
              <div>
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Số điện thoại</span>
                <p className="text-sm text-gray-800 font-bold flex items-center gap-1.5">
                  <Phone size={13} className="text-gray-400" /> {order.shippingPhone}
                </p>
              </div>
              <div>
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Địa chỉ</span>
                <p className="text-sm text-gray-600 font-semibold leading-relaxed flex items-start gap-1.5">
                  <MapPin size={13} className="text-gray-400 mt-0.5 shrink-0" /> {order.shippingAddress}
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden p-6 space-y-4">
            <h4 className="text-sm font-bold text-gray-700 ml-1 flex items-center gap-2 border-b pb-3">
              <CreditCard size={14} className="text-primary" /> Thông tin thanh toán
            </h4>
            <div className="space-y-3">
              <div>
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Phương thức</span>
                <p className="text-sm font-bold text-gray-800">
                  {order.payment?.method === 'BANK_TRANSFER' ? 'Chuyển khoản' : 'Thanh toán khi nhận hàng (COD)'}
                </p>
              </div>
              <div>
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Trạng thái</span>
              ...
              {order.payment?.paidAt && (
                <div>
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Thời gian thanh toán</span>
                  <p className="text-sm font-bold text-gray-500 italic">
                    {format(new Date(order.payment.paidAt), "HH:mm:ss dd/MM/yyyy", { locale: vi })}
                  </p>
                </div>
              )}
            </div>
